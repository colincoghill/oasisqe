...
# Sets up Ubuntu (Trusty) to run OASIS self-tests. And then runs them
#
...
/bin/bash ${SRC}/src/build/install_systemdeps_trusty.sh
...
sed -i "s/statichost: http:\/\/localhost/statichost: http:\/\/localhost:8088/g" /etc/oasisqe.ini
sed -i "s/url: http:\/\/localhost\/oasis/url: http:\/\/localhost:8088\/oasis/g" /etc/oasisqe.ini
...
echo
echo
echo OASISQE deployed to http://localhost:8088/oasis
echo
echo "********************************************"
