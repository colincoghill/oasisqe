...
# Sets up Debian (Stretch) to run OASIS self-tests. And then runs them
#
...
sed -i "s/statichost: http:\/\/localhost/statichost: http:\/\/localhost:8087/g" /etc/oasisqe.ini
sed -i "s/url: http:\/\/localhost\/oasis/url: http:\/\/localhost:8087\/oasis/g" /etc/oasisqe.ini
...
echo
echo
echo OASISQE deployed to http://localhost:8087/oasis
echo
echo "********************************************"
